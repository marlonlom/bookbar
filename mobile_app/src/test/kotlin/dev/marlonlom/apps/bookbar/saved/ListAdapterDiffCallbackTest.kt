...
class ListAdapterDiffCallbackTest : TestCase() {
    ...
    private val diffCallback = SavedBooksListAdapter.DiffCallback()

    private fun prepareSavedBookDetail(response: BookDetailApiResponse) =
        toBookDetailEntity(response).copy(saved = true)

    @Test
    fun `Should return false when comparing the contents of two items`() {
        val oldItem = prepareSavedBookDetail(RemoteData.savedBooksApiResponse.books[0])
        val newItem = prepareSavedBookDetail(RemoteData.savedBooksApiResponse.books[1])
